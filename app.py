...
<style>
    ...
    .result-label {
        font-size: 0.75rem; color: #8a8894;
...
if not model_loaded:
    st.warning("⚠️ **Model files not found.** Place `best_model.pkl` and `scaler.pkl` and `selected_features.pkl` in the same directory.", icon="⚠️")
...
with c4:
    Diabetes = st.selectbox("Diabetes", [0, 1, 2],
        format_func=lambda x: {0: "No", 1: "Pre-diabetic", 2: "Diabetic"}[x])
...
if predict_btn:
    input_data = np.array([[
        ...
        Diabetes, PhysActivity, GenHlth, Age, Income
    ]], dtype=float)
